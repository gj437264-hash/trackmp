    ...
    <DashboardLayout>
      <div className="p-6 md:p-10">
        <div className="flex items-center justify-between flex-wrap gap-4">
          <div>
            <div className="soft-label mb-0">/// Recoverable Records</div>
            <h1 className="mt-2 font-display font-black text-4xl text-slate-900">Trash</h1>
          </div>
          <div className="flex items-center gap-2">
            <select value={type} onChange={(e) => setType(e.target.value)} className="soft-input text-sm w-auto" data-testid="trash-type">
              {ENTITY_TYPES.map((t) => <option key={t} value={t}>{t || "All types"}</option>)}
            </select>
            <button onClick={load} className="btn-soft-secondary text-xs px-3 py-2"><RefreshCw size={12} /></button>
          </div>
        </div>
        ...
        <div className="mt-6 soft-table-wrap overflow-x-auto">
          <table className="w-full border-collapse soft-table">
            ...
            <tbody>
              {loading && <tr><td colSpan={4} className="p-10 text-center text-slate-400 uppercase text-xs font-bold tracking-wider">Loading…</td></tr>}
              {!loading && !items.length && <tr><td colSpan={4} className="p-10 text-center text-slate-400 uppercase text-xs font-bold tracking-wider">Trash is empty</td></tr>}
              {items.map((it) => {
                const displayName = String(it.record[NAME_FIELDS[it.entity_type]] ?? it.entity_id);
                return (
                  <tr key={it.entity_id} data-testid={`trash-row-${it.entity_id}`}>
                    <td className="font-mono text-xs uppercase">{it.entity_type}</td>
                    <td className="font-bold">{displayName}</td>
                    <td className="font-mono text-xs">{it.record.deleted_at ? new Date(it.record.deleted_at).toLocaleString() : "—"}</td>
                    <td>
                      <div className="flex gap-2">
                        <button data-testid={`restore-${it.entity_id}`} onClick={() => restore(it)} className="btn-soft-secondary text-xs px-3 py-1.5">
                          <RotateCcw size={12} className="mr-1" /> Restore
                        </button>
                        <button data-testid={`purge-${it.entity_id}`} onClick={() => purge(it)} className="btn-soft-danger px-3 py-1.5">
